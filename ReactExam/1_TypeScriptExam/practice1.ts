...
let num = 10; //자바 스크립트에서는 이걸 숫자로 넘기든 문자로 넘기든 전혀 상관 없음.

let num2:number = 10; //타입스크립트는 이렇게 체크를 해줌. 여기에 문자넣으면 오류남.
...
//array, object, enum
//서로다른 데이터를 묶을수 있음 
const arr_num : number[] = [1, 2, 3]; //숫자로 집어넣을건데 배열형태로 넣을것이다. number[]. 이렇게 체크함.
const arr_bool : boolean[] = [true, false]; //여긴 boolean형태만 넣을 수 있게.
...
//------------------------------------------------------------------//
//any, unknown
let anyValue : any = 1; //: any라고하면 어떤값이든 가능함. 체크를 안함.
...
function hello() : void{ //return값이 없을 때는 void.
    console.log("Hello");
    //여기서 return쓰면 에러남.
}

function hello2() : String { //리턴값이 있는 경우, 리턴값의 형태가 무엇인지 적어줌.
    return "hello";
}
...
//------------------------------------------------------------------//
//null, undefined
const nullValue : null = null; // 반드시 null값만 집어넣어야함.
